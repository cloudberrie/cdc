...
implements Cloneable
{
  ...
  private static final int isNewRecordoffSet=0;
  private static final int isNewRecLength=4;
  private static final int VersionOffset = isNewRecordoffSet+isNewRecLength;
  ...
  private static final int INSERT_MASK               = 0x0008;  
  ...
  private static final byte[] InsertLongKeyAttributesBigEndian = new byte[DbusEventV1.AttributesLength];
  private static final byte[] InsertStringKeyAttributesBigEndian = new byte[DbusEventV1.AttributesLength];
  
  private static final byte[] InsertLongKeyAttributesLittleEndian = new byte[DbusEventV1.AttributesLength];
  private static final byte[] InsertStringKeyAttributesLittleEndian = new byte[DbusEventV1.AttributesLength];
  ...
  static
  {
    ...
    setOpcode(InsertLongKeyAttributesBigEndian, DbusOpcode.INSERT, ByteOrder.BIG_ENDIAN);
    setOpcode(InsertStringKeyAttributesBigEndian, DbusOpcode.INSERT, ByteOrder.BIG_ENDIAN);
    setKeyTypeString(InsertStringKeyAttributesBigEndian, ByteOrder.BIG_ENDIAN);
    ...
    setOpcode(InsertLongKeyAttributesLittleEndian, DbusOpcode.INSERT, ByteOrder.LITTLE_ENDIAN);
    setOpcode(InsertStringKeyAttributesLittleEndian, DbusOpcode.INSERT, ByteOrder.LITTLE_ENDIAN);
    setKeyTypeString(InsertStringKeyAttributesLittleEndian, ByteOrder.LITTLE_ENDIAN);
                                           ...
                                           DbusEventInfo eventInfo)
  {
    ...
    } else if (eventInfo.getOpCode() == DbusOpcode.UPSERT){
      if (serializationBuffer.order() == ByteOrder.BIG_ENDIAN)
    	...
    	if (serializationBuffer.order() == ByteOrder.BIG_ENDIAN)
            attributes = InsertLongKeyAttributesBigEndian.clone();
          else
            attributes = InsertLongKeyAttributesLittleEndian.clone();
    }
                                        ...
                                        byte[] attributes)
  {
    ...
    serializationBuffer.getInt(0);
    
    int startPosition = serializationBuffer.position();
    if(eventInfo.isInsertEvent()){
    	serializationBuffer.putInt(1);
    }else{
    	serializationBuffer.putInt(0);
    } 
    serializationBuffer.getInt(0);
    ...
    Utils.putUnsignedInt(serializationBuffer, startPosition+ValueCrcOffset, valueCrc);
     stopPosition = serializationBuffer.position();
  ...
  private static void setOpcode(byte[] attribute, DbusOpcode opcode, ByteOrder byteOrder)
  {
    switch (opcode)
    {
      ...
      case DELETE:
        ...
        break;
      case INSERT:
          setAttributeBit(attribute, INSERT_MASK, byteOrder);
          break;
      default:
  ...
  public DbusOpcode getOpcode()
  {
    ...
    if (isAttributeBitSet(INSERT_MASK))
    {
      return DbusOpcode.INSERT;
    }
    return null;
  }
...
@Override
public boolean isInsertDbEvent() {
	
	int isNewRec=_buf.getInt(_position+isNewRecordoffSet);
	if(isNewRec==1){
		return true;
	}
	return false;
