...
class ORListener extends DatabusThreadBase implements BinlogEventListener
{
  ...
  {
    frameAvroRecord(dre.getTableId(), dre.getHeader(), dre.getRows(), DbusOpcode.DELETE,false);
  }
  ...
  private void updateRows(UpdateRowsEvent ure)
  {
    ...
    frameAvroRecord(ure.getTableId(), ure.getHeader(), lr, DbusOpcode.UPSERT,false);
  }
  ...
  {
	  _log.info("Converting Insert Event1 to Avro Record...");
    frameAvroRecord(wre.getTableId(), wre.getHeader(), wre.getRows(), DbusOpcode.UPSERT,true);
  }

  private void insertRows(WriteRowsEventV2 wre)
  {
	  _log.info("Converting Insert Event2 to Avro Record...");
    frameAvroRecord(wre.getTableId(), wre.getHeader(), wre.getRows(), DbusOpcode.UPSERT,true);
  }

  private void frameAvroRecord(long tableId, BinlogEventV4Header bh, List<Row> rl, final DbusOpcode doc,Boolean isInsertDBEvent)
  {
	  _log.info("isInsertDBEvent:"+isInsertDBEvent);
    try
    {
      ...
      for(Row r: rl)
      {
        ...
        db.setInsertDBEvent(isInsertDBEvent);
      ...
      throws DatabusException
  {
    ...
    {
      _log.info("Generic record = " + record);
    }
  }
