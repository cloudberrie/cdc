...
import com.linkedin.databus.client.data_model.DBRecord;
import com.linkedin.databus.client.data_model.DBRecordInfo;
import com.linkedin.databus.client.data_model.FieldValues;
...
import com.linkedin.databus.client.data_model.Field;
...
public class DbusEventAvroDecoder implements DbusEventDecoder
{
  ...
  {
	     createDBRecord(e,out);
//	    byte[] md5 = new byte[16];
//	    e.schemaId(md5);
//	    SchemaId schemaId = new SchemaId(md5);
//	    VersionedSchema sourceSchema = _schemaSet.getById(schemaId);
//		//VersionedSchema sourceSchema =getMetadataSchema(e);
//		LOG.debug("kishore:"+e.getClass().getName());
//		
//	    ByteBuffer valueBuffer = e.value();
//	    byte[] valueBytes = new byte[valueBuffer.remaining()];		
// 
...
//	      binDecoder.set(DecoderFactory.defaultFactory().createBinaryDecoder(valueBytes, binDecoder.get()));
//	      GenericRecord  datum = (GenericRecord)reader.read(null, binDecoder.get());
//	      LOG.error("kishoreasdfasdf:"+datum.getClass().getName());
//	      LOG.error("kishoreasdfasdf:"+datum);
//	      LOG.error("kishorr first_name:"+datum.get("id"));
//		   
...
//	    }
	...
	public void createDBRecord(DbusEvent e, OutputStream out) {
		...
		try {
			...
			GenericRecord decodedEvent = getGenericRecord(e, null);
			for (org.apache.avro.Schema.Field field : fields) {
				String fieldName = field.name();
				String fieldMeta = field.getProp("meta");
				String[] metaTokens = fieldMeta.split(";");
				String[] DbFieldColInfo = metaTokens[0].split("=");
				String dbColName = DbFieldColInfo[1];

				String[] DbFieldIndexInfo = metaTokens[1].split("=");
				String dbIndexPosition = DbFieldIndexInfo[1];
				
				String[] DbFieldTypeInfo = metaTokens[3].split("=");
				String javaObjType = DbFieldTypeInfo[1];
				
				
//				String javaObjType=field.getProp("java-class");

				Object value=decodedEvent.get(fieldName);
				
				String fieldType = value.getClass().getSimpleName();
				Field field1 = new Field(fieldName, dbColName,
						Integer.valueOf(dbIndexPosition), fieldType,javaObjType);

				fieldList.add(field1);

				FieldValues fieldValues = new FieldValues(fieldName, value.toString());
				fieldValuesList.add(fieldValues);
			}

			DBRecordInfo recordInfo = new DBRecordInfo();

			recordInfo.setDbFields(fieldList);
			recordInfo.setFieldValues(fieldValuesList);

			String schemaMetaInfo = schema.getProp("meta");

			String[] metaFields = schemaMetaInfo.split(";");

			String tableNameMetaInfo = metaFields[0];
			String[] tableInfo = tableNameMetaInfo.split("=");
			String tableName = tableInfo[1];

			String tablePrimaryKeyInfo = metaFields[3];
			String pkInfo[] = tablePrimaryKeyInfo.split("=");

			recordInfo.setTableName(tableName);
			recordInfo.setPrimaryKey(pkInfo[1]);

			DBRecord record = new DBRecord();

			record.setDbRecordInfo(recordInfo);
			if(e.isInsertDbEvent()){
				record.setOperation("INSERT");
			}else{
				record.setOperation(e.getOpcode().toString());
			}
			
			record.setSourceId(String.valueOf(e.getSourceId()));
			
			ObjectMapper mapperObj = new ObjectMapper();
			
			String jsonStr = mapperObj.writeValueAsString(record);
			...
			ex.printStackTrace();
			LOG.error("event value serialization error; event = " + ex, ex);
		}
