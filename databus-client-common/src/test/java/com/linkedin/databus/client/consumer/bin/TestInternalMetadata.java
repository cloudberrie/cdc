...
//    TestUtil.setupLogging(true, "TestInternalMetadata-" + dateFormatter.format(now) + ".log", Level.ERROR);
  ...
  public static void main(String args[]){
	  ...
	  try{
		    TestInternalMetadata test=new TestInternalMetadata();
		    DbusEventPart metadataPart = test.createMetadataPart();
		    DbusEvent event = test.createEvent(metadataPart);
		    
		    System.out.println("kishore: "+event.getClass().getName());
		    System.out.println("kishore: "+event.getOpcode());
	  }catch(Exception e){
		  e.printStackTrace();
	  }
